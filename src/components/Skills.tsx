            ...
            return (
              <div key={index} className="skill-row-trigger relative group w-full flex flex-col">
                ...
                <div className="relative z-10 border-t border-white/20 pt-8 mt-8">
                  {/* Typing effect text reveal container */}
                  <h4 className="skill-title-container flex flex-wrap font-serif text-[clamp(2.5rem,4vw,5rem)] text-white leading-none tracking-tight mb-12 drop-shadow-[0_0_15px_rgba(0,0,0,0.5)]">
                    {title.split('').map((char, index) => (
                      <span key={index} className="skill-title-char opacity-0 translate-y-4 group-hover:text-arcane-glow transition-colors duration-300">
                        {char === ' ' ? '\u00A0' : char}
                      </span>
                    ))}
                  </h4>
